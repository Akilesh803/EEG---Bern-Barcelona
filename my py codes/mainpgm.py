...
def main():
    ...
    res2=[]
    res3=[]
    path="D:\\project\\FN EEG Dataset focal\\"
    for filename in os.listdir(path):
        path1=path+filename+"\\"
        for file_name in os.listdir(path1):
            print(file_name)
            data=[]
            with open(path1+"{}".format(file_name)) as f:
    ...
    np.savetxt("Butterworth applied signals x.csv", res1, delimiter=",")
    np.savetxt("Butterworth applied signals y.csv",res2,delimiter=",")
    np.savetxt("Butterworth applied signals z.csv",res3,delimiter=",")
